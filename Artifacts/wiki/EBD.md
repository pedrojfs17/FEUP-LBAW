...
The diagram of Figure 1 presents the main entities, the relationships between them, attributes and their domains, and the multiplicity of relationships for Oversee.

### 1. Class diagram

![Class Diagram](https://i.imgur.com/Y1aqDXS.png)

Figure 1: Oversee's class diagram

### 2. Additional Business Rules
 
- Tags created in a project can only be used in tasks within the same project.
...
- There can't be recursive relations between tasks(e.g., `task1` has subtask `task2` and `task2` has subtask `task1`)
- A task cannot wait for its parent task.

---

## A5: Relational Schema, validation and schema refinement

This artefact contains the Relational Schema obtained from the Conceptual Data Model presented in the previous artefact. It also contains the Schema's validation and the SQL code to generate the Database.

### 1. Relational Schema

| Relation reference | Relation Compact Notation                        |
| ------------------ | ------------------------------------------------ |
| R01                | account(<ins>id</ins>, username **UK NN**, e-mail **UK NN**, password **NN**, is_admin **NN**)|
| R02                | password_resets(email **NN**, token **NN**, created_at **NN**)|
| R03                | country(<ins>id</ins>, iso **NN**, name **NN**)|
| R04                | client(<ins>id</ins> → account **NN**, fullname, company, avatar, color **NN**, client_gender **DF** 'Unspecified' **CK** (client_gender **IN** Gender), country → country, allowNoti **NN**, inviteNoti **NN**, memberNoti **NN**, assignNoti **NN**, waitingNoti **NN**, reportNoti **NN**, commentNoti **NN**, hideCompleted **NN**, simplifiedTasks **NN**, search)|
| R05                | project(<ins>id</ins>, name **NN**, description **NN**, due_date **CK** (due_date > CURRENT_DATE), search)|
| R06                | team_member(<ins>client_id</ins> → client **NN**, <ins>project_id</ins> → project **NN**, member_role **NN** **CK** (member_role **IN** Role))|
| R07                | task(<ins>id</ins>, project → project **NN**, name **NN**, description, due_date, task_status **CK** (task_status **IN** Status), <ins>parent</ins> → task, search)|
| R08                | waiting_on(<ins>task1</ins> → task, <ins>task2</ins> → task) |
| R09                | assignment(<ins>task</ins> → task, <ins>client</ins> → client) |
| R10                | tag(<ins>id</ins>, project → project **NN**, name **NN**, color **NN**) |
| R11                | contains_tag(<ins>tag</ins> → tag, <ins>task</ins> → task) |
| R12                | check_list_item(<ins>id</ins>, item_text **NN**, completed **NN**, task → task **NN**) |
| R13                | comment(<ins>id</ins>, comment_text, task → task **NN**, author → client, comment_date **NN**, <ins>parent</ins> → comment) |
| R14                | social_media_account(<ins>id</ins>, social_media  **CK** (social_media **IN** Website) **NN**, username **NN**, accessToken **NN**) |
| R15                | associated_project_account(<ins>account</ins> → social_media_account, <ins>project</ins> → project) |
| R16                | associated_client_account(<ins>account</ins> → social_media_account, <ins>client</ins> → client) |
| R17                | report(<ins>id</ins>, report_text **NN**, reporter → client **NN**, reported → client **NN**, statee **NN**) |
| R18                | notification(<ins>id</ins>, client → client **NN**, seen **NN**, notification_text **NN**, notification_date **NN**) |
| R19                | comment_notification(<ins>id</ins> → notification **NN**, comment → comment **NN**) |
| R20                | assignment_notification(<ins>id</ins> → notification **NN**, assignment → task **NN**) |
| R21                | project_notification(<ins>id</ins> → notification **NN**, project → project **NN**) |
| R22                | report_notification(<ins>id</ins> → notification **NN**, report → report **NN**) |
| R23                | invite(<ins>client_id</ins> → client, <ins>project_id</ins> → project, decision) |
| R24                | user_support(<ins>id</ins>, email **NN**, name, subject **NN**, body **NN**, responded **NN**, response) |

### 2. Domains

| Domain Name  | Domain Specification           |
| -----------  | ------------------------------ |
| Status       | ENUM ('Waiting', 'Not Started', 'In Progress', 'Completed') |
| Gender       | ENUM ('Male', 'Female', 'Unspecified') |
| Role         | ENUM ('Owner', 'Editor', 'Reader') |
| Website      | ENUM ('Facebook', 'Instagram', 'Twitter') |
| Report State | ENUM ('Pending', 'Ignored', 'Banned') |
...
| FD0101          | id → {username, e-mail, password, is_admin} |
| FD0102          | username → {id, e-mail, password, is_admin} |
| FD0103          | e-mail → {id, username, password, is_admin} |
| **NORMAL FORM** | BCNF               |

| **TABLE R02**   | password_resets               |
| --------------  | ---                |
| **Keys**        | { id }             |
| **Functional Dependencies:**  | id → {email, token, created_at}|
| **NORMAL FORM** | BCNF               |
...
| **Keys**        | { id }             |
| **Functional Dependencies:**        |
| FD0401          | id → {fullname, company, avatar, color, client_gender, country, allowNoti, inviteNoti, memberNoti, assignNoti, waitingNoti, reportNoti, commentNoti, hideCompleted, simplifiedTasks, search} |
| **NORMAL FORM** | BCNF               |
...
| **Keys**        | { id }             |
| **Functional Dependencies:**        |
| FD0501          | id → {name, description, due_date, search} |
| **NORMAL FORM** | BCNF               |

| **TABLE R06**   | team_member               |
| --------------  | ---                |
| **Keys**        | { client_id, project_id }             |
| **Functional Dependencies:** |
| FD0601          | (client_id, project_id) → {role} |
| **NORMAL FORM** | BCNF               |
...
| **Keys**        | { id }             |
| **Functional Dependencies:**        |
| FD0701          | id → {project, name, description, due_date, task_status, parent, search} |
| **NORMAL FORM** | BCNF               |

| **TABLE R08**   | waiting_on      |
| --------------  | ---                |
| **Keys**        | { task1, task2 }             |
| **Functional Dependencies:**        | none |
| **NORMAL FORM** | BCNF               |

| **TABLE R09**   | assignment      |
| --------------  | ---                |
| **Keys**        | { task, client }             |
| **Functional Dependencies:**        | none |
| **NORMAL FORM** | BCNF               |
...
| **Keys**        | { id }             |
| **Functional Dependencies:**        |
| FD1101          | id → {project, name, color} |
| **NORMAL FORM** | BCNF               |

| **TABLE R11**   | contains_tag      |
| --------------  | ---                |
| **Keys**        | { tag, task }             |
| **Functional Dependencies:**        | none |
| **NORMAL FORM** | BCNF     |

| **TABLE R12**   | check_list_item      |
| --------------  | ---                |
| **Keys**        | { id }             |
| **Functional Dependencies:**        |
| FD1301          | id → {item_text, completed, task} |
| **NORMAL FORM** | BCNF               |
...
| **Keys**        | { id }             |
| **Functional Dependencies:**        |
| FD1401          | id → {comment_text, task, author, comment_date, parent} |
| **NORMAL FORM** | BCNF               |

| **TABLE R14**   | social_media_account      |
| --------------  | ---                |
| **Keys**        | { id }             |
| **Functional Dependencies:**        |
| FD1601          | id → {social_media, username, access_token} |
| **NORMAL FORM** | BCNF               |

| **TABLE R15**   | associated_project_account      |
| --------------  | ---                |
| **Keys**        | { account, project } |
| **Functional Dependencies:**        | none |
| **NORMAL FORM** | BCNF               |

| **TABLE R16**   | associated_client_account      |
| --------------  | ---                |
| **Keys**        | { account, client } |
| **Functional Dependencies:**        | none |
| **NORMAL FORM** | BCNF               |
...
| **Keys**        | { id }             |
| **Functional Dependencies:**        |
| FD1901          | id → {report_text, state, reporter, reported} |
| **NORMAL FORM** | BCNF               |
...
| **Keys**        | { id }             |
| **Functional Dependencies:**        |
| FD2001          | id → {client, seen, notification_text, notification_date} |
| **NORMAL FORM** | BCNF               |

| **TABLE R19**   | comment_notification      |
| --------------  | ---                |
| **Keys**        | { id }             |
| **Functional Dependencies:**        |
| FD2101          | id → {comment} |
...
| **TABLE R20**   | assignment_notification      |
| --------------  | ---                |
| **Keys**        | { id }             |
| **Functional Dependencies:**        |
| FD2201          | id → {assignment} |
| **NORMAL FORM** | BCNF               |

| **TABLE R21**   | project_notification      |
| --------------  | ---                |
| **Keys**        | { id }             |
| **Functional Dependencies:**        |
| FD2301          | id → {project} |
...
| **Keys**        | { client_id, project_id }             |
| **Functional Dependencies:**        |
| FD2601          | client_id, project_id → {decision} |
| **NORMAL FORM** | BCNF               |

| **TABLE R24**   | user_support      |
| --------------  | ---                |
| **Keys**        | { id }             |
| **Functional Dependencies:**        |
| FD2701          | id → {email, name, subject, body, responded, response} |
| **NORMAL FORM** | BCNF               |
...
This artefact also contains the complete database creation script, including indexes and triggers.
...
| --- | -------------------------- | --------------------- | ------------------ |
| R01 | account                    | dozens of thousands   | dozens per day     |
| R02 | password_resets            | thousands             | dozens per motnh   |
| R03 | country                    | hundreds              | no growth          |
| R04 | client                     | dozens of thousands   | dozens per day     |
| R05 | project                    | dozens of thousands   | dozens per day     |
| R06 | team_member                | hundreds of thousands | dozens per day     |
| R07 | task                       | hundreds of thousands | hundreds per day   |
| R08 | waiting_on                 | dozens of thousands   | dozens per day   |
| R09 | assignment                 | hundreds of thousands | hundreds per day   |
| R10 | tag                        | hundreds of thousands | hundreds per day   |
| R11 | contains_tag               | hundreds of thousands | hundreds per day   | 
| R12 | check_list_item            | millions              | thousands per day   |
| R13 | comment                    | hundreds of thousands | hundreds per day   |
| R14 | social_media_account       | thousands             | dozens per month   |
| R15 | associated_project_account | thousands             | dozens per month   |
| R16 | associated_client_account  | thousands             | dozens per month   |
| R17 | report                     | hundreds              | dozens per year   |
| R18 | notification               | millions              | thousands per day   |
| R19 | comment_notification       | hundreds of thousands | hundreds per day   |
| R20 | assignement_notification   | hundreds of thousands | hundreds per day   |
| R21 | project_notification       | hundreds of thousands | hundreds per day   |
| R22 | report_notification        | hundreds              | dozens per year |
| R23 | invite                     | hundreds of thousands | hundreds per day |
| R24 | user_support               | dozens                | units per year |
...
<table>
	<tbody>
        ...
        <tr>
            <th> Description </th>
            <td> Account by username or email </td>
        </tr>
        <tr>
            <th> Frequency </th>
...
SELECT *
FROM account
WHERE username = $input OR email = $input;</td>
        </tr>
    </tbody>
</table>
...
<table>
	<tbody>
        ...
        <tr>
            <td colspan="2"><pre>
SELECT account.username, account.email, client.fullname, client.company, client.gender, country.name
FROM client, account, country
WHERE account.id = $user_id 
    AND account.id = client.id 
    AND country.id = client.country;</td>
        </tr>
    </tbody>
</table>
...
<table>
	<tbody>
        ...
        <tr>
            <td colspan="2"><pre>
SELECT color, allowNoti, inviteNoti, memberNoti, assignNoti, waitingNoti, reportNoti, commentNoti, hideCompleted, simplifiedTasks
FROM client
WHERE id = $user_id;</td>
        </tr>
    </tbody>
...
<table>
	<tbody>
        ...
        <tr>
            <td colspan="2"><pre>
SELECT projects.*, 
    round(avg((task_status = 'Completed')::int) * 100) 
        AS completion
FROM (SELECT project.*
      FROM project 
          JOIN team_member ON project.id = team_member.project_id
      WHERE team_member.client_id = $client_id
     ) AS projects 
  JOIN task ON projects.id = task.project
GROUP BY projects.id, 
    projects.name, 
    projects.description, 
    projects.due_date;</td>
        </tr>
    </tbody>
</table>
...
SELECT *
FROM task
WHERE task.project = $project_id;</td>
        </tr>
    </tbody>
</table>
...
<table>
	<tbody>
        ...
        <tr>
            <td colspan="2"><pre>
SELECT task.*
FROM task, contains_tag
WHERE task.project = $project_id 
    AND contains_tag.tag = $tag 
    AND contains_tag.task = task.id;</td>
        </tr>
    </tbody>
</table>
...
<table>
	<tbody>
        ...
        <tr>
            <td colspan="2"><pre>
SELECT task.* 
FROM task, waiting_on
WHERE waiting_on.task1 = $task_id 
    AND task.id = waiting_on.task2;</td>
        </tr>
    </tbody>
</table>
...
<table>
	<tbody>
        ...
        <tr>
            <td colspan="2"><pre>
SELECT client.fullname, team_member.role
FROM client, project, team_member
WHERE client.id = team_member.client_id 
    AND project.id = team_member.project_id;</td>
        </tr>
    </tbody>
</table>
...
SELECT comment.*
FROM task, comment
WHERE comment.task_id = task.id;</td>
        </tr>
    </tbody>
</table>
...
FROM task, assignment, client
WHERE assignment.task = task.id AND assignment.client=client.id;</td>
        </tr>
    </tbody>
</table>
...
<table>
	<tbody>
        ...
        <tr>
            <td colspan="2"><pre>
SELECT tag.name, project.name
FROM project, tag
WHERE tag.project = project.id;</td>
        </tr>
    </tbody>
</table>
...
<table>
	<tbody>
        ...
        <tr>
            <th> Description </th>
            <td> Social Media Client's Accounts </td>
        </tr>
        <tr>
            <th> Frequency </th>
            <td> hundreds per day </td>
        ...
        <tr>
            <td colspan="2"><pre>
SELECT acc.id, acc.username, acc.social_media
FROM social_media_account AS acc 
    JOIN associated_client_account AS ass ON acc.id = ass.account
WHERE ass.client = $client_id;</td>
        </tr>
    </tbody>
</table>
...
<table>
	<tbody>
        ...
        <tr>
            <th> Description </th>
            <td> Social Media Project's Accounts </td>
        </tr>
        <tr>
            <th> Frequency </th>
            <td> dozens per day </td>
...
SELECT acc.id, acc.username, acc.social_media
FROM social_media_account AS acc 
    JOIN associated_project_account AS ass ON acc.id = ass.account
WHERE ass.project = $project_id;</td>
        </tr>
    </tbody>
</table>
...
SELECT notification.*, 
	...
	assignment_notification.assignment,
	project_notification.project,
	report_notification.report
FROM notification 
    LEFT JOIN comment_notification 
        ON notification.id = comment_notification.id
    LEFT JOIN assignment_notification 
        ON notification.id = assignment_notification.id
    LEFT JOIN project_notification 
        ON notification.id = project_notification.id
    LEFT JOIN report_notification 
        ON notification.id = report_notification.id
WHERE notification.client = $client_id;</td>
        </tr>
    </tbody>
</table>
...
<table>
	<tbody>
        ...
        <tr>
            <th> Description </th>
            <td> Search for a client, using his username, email, fullname or company </td>
        </tr>
        <tr>
            <th> Frequency </th>
            <td> dozens per day </td>
...
SELECT *
FROM client
WHERE search @@ plainto_tsquery(concat($search_term,':*'))
ORDER BY ts_rank(search, plainto_tsquery(concat($search_term,':*'))) DESC</td>
        </tr>
    </tbody>
</table>
...
SET color = $color, 
    allowNoti = $allowNoti, 
    inviteNoti = $inviteNoti, 
    memberNoti = $memberNoti, 
    assignNoti = $assignNoti, 
    waitingNoti = $waitingNoti, 
...
VALUES ($username, $email, $password);</td>
        </tr>
    </tbody>
</table>
...
<table>
	<tbody>
        ...
        <tr>
            <td colspan="2"><pre>
INSERT INTO project (name, description, due_date)
VALUES ($name, $description, $due_date);</td>
        </tr>
    </tbody>
</table>
...
<table>
	<tbody>
        ...
        <tr>
            <td colspan="2"><pre>
INSERT INTO task (project, name, description, due_date, task_status, parent)
VALUES ($project, $name, $description, $due_date, 'Not Started', $task);</td>
        </tr>
    </tbody>
</table>
...
<table>
	<tbody>
        ...
        <tr>
            <td colspan="2"><pre>
INSERT INTO team_member (client_id, project_id, role)
VALUES ($client_id, $project_id, $role);</td>
        </tr>
    </tbody>
</table>
...
<table>
	<tbody>
        ...
        <tr>
            <th> Description </th>
            <td> Remove Team Member from Project </td>
        </tr>
        <tr>
            <th> Frequency </th>
            <td> hundreds per day </td>
...
WHERE client_id = $client_id AND project_id = $project_id;</td>
        </tr>
    </tbody>
</table>
...
<table>
	<tbody>
        ...
        <tr>
            <th> Justification </th>
            <td> Both Queries must be fast as they are executed many times; doesn't need range query support; cardinality is medium because client_id can be repeated; it's a good candidate for clustering because it has medium cardinality and the tuple is small. </td>
        </tr>
        <tr>
            <td colspan="2"><pre>
CREATE INDEX client_member_index ON team_member USING hash(client_id);</td>
        </tr>
    </tbody>
</table>
...
<table>
	<tbody>
        ...
        <tr>
            <th> Justification </th>
            <td> 	Both Queries must be fast as they are executed many times; doesn't need range query support; cardinality is medium because project_id can be repeated; it's not a good candidate for clustering because clustering is made with client_id. </td>
        </tr>
        <tr>
            <td colspan="2"><pre>
CREATE INDEX project_member_index ON team_member USING hash(project_id);</td>
        </tr>
    </tbody>
</table>
...
<table>
	<tbody>
        ...
        <tr>
            <th> Related queries </th>
            <td> SELECT04, SELECT05, SELECT06 </td>
        </tr>
        <tr>
            <th> Relation </th>
            <td> task </td>
        ...
        <tr>
            <th> Justification </th>
            <td> All three Queries must be fast as they are executed many times; doesn't need range query support; cardinality is medium because project can be repeated; it's not a good candidate for clustering because the tuple is not small. </td>
        </tr>
        <tr>
            <td colspan="2"><pre>
CREATE INDEX task_index ON task USING hash(project);</td>
        </tr>
    </tbody>
</table>
...
<table>
	<tbody>
        ...
        <tr>
            <th> Justification </th>
            <td> Query SELECT09 must be fast as it is executed many times; doesn't need range query support; cardinality is medium because task1 can be repeated; it's a good candidate for clustering because cardinality is medium and the tuple is small. </td>
        </tr>
        <tr>
            <td colspan="2"><pre>
CREATE INDEX waiting_index ON waiting_on USING hash(task1);</td>
        </tr>
    </tbody>
</table>
...
<table>
	<tbody>
        ...
        <tr>
            <th> Justification </th>
            <td> Query SELECT12 must be fast as it is executed many times; doesn't need range query support; cardinality is medium because task can be repeated; it's a good candidate for clustering because cardinality is medium and the tuple is small. </td>
        </tr>
        <tr>
            <td colspan="2"><pre>
CREATE INDEX task_assign_index ON assignment USING hash(task);</td>
        </tr>
    </tbody>
</table>
...
<table>
	<tbody>
        ...
        <tr>
            <th> Justification </th>
            <td> Query SELECT12 must be fast as it is executed many times; doesn't need range query support; cardinality is medium because clients can be repeated for different tasks; it isn't a good candidate for clustering because this table is already clustered with INDEX08. </td>
        </tr>
        <tr>
            <td colspan="2"><pre>
CREATE INDEX client_assign_index ON assignment USING hash(task);</td>
        </tr>
    </tbody>
</table>
...
<table>
	<tbody>
        ...
        <tr>
            <th> Justification </th>
            <td> Query SELECT13 must be fast as it is executed many times; doesn't need range query support; cardinality is medium because task can be repeated; it's a good candidate for clustering because cardinality is medium and the tuple is small. </td>
        </tr>
        <tr>
            <td colspan="2"><pre>
CREATE INDEX tag_index ON tag USING hash(project);</td>
        </tr>
    </tbody>
</table>
...
<table>
	<tbody>
        ...
        <tr>
            <th> Justification </th>
            <td> Query SELECT06 must be fast as it is executed many times; doesn't need range query support; cardinality is medium because task can be repeated; it's a good candidate for clustering because cardinality is medium and the tuple is small. </td>
        </tr>
        <tr>
            <td colspan="2"><pre>
CREATE INDEX task_tag_index ON contains_tag USING hash(task);</td>
        </tr>
    </tbody>
</table>
...
<table>
	<tbody>
        ...
        <tr>
            <th> Justification </th>
            <td> Query SELECT06 must be fast as it is executed many times; doesn't need range query support; cardinality is medium because tag can be repeated; it's not a good candidate for clustering because clustering is done with task. </td>
        </tr>
        <tr>
            <td colspan="2"><pre>
CREATE INDEX tag_task_index ON contains_tag USING hash(tag);</td>
        </tr>
    </tbody>
</table>
...
<table>
	<tbody>
        ...
        <tr>
            <th> Justification </th>
            <td> Query SELECT07 must be fast as it is executed many times; doesn't need range query support; cardinality is medium because task can be repeated; it's a good candidate for clustering because cardinality is medium and the tuple is small. </td>
        </tr>
        <tr>
            <td colspan="2"><pre>
CREATE INDEX check_list_index ON check_list_item USING hash(task);</td>
        </tr>
    </tbody>
</table>
...
<table>
	<tbody>
        ...
        <tr>
            <th> Justification </th>
            <td> Query SELECTXX must be fast as it is executed many times; doesn't need range query support; cardinality is medium because task can be repeated; it's a good candidate for clustering because cardinality is medium and the tuple is small. </td>
        </tr>
        <tr>
            <td colspan="2"><pre>
CREATE INDEX comment_index ON comment USING btree(task, comment_date);</td>
        </tr>
    </tbody>
</table>
...
<table>
	<tbody>
        ...
        <tr>
            <th> Justification </th>
            <td> Query SELECT15 must be fast as it is executed many times; doesn't need range query support; cardinality is medium because client can be repeated; it's a good candidate for clustering because cardinality is medium and the tuple is small. </td>
        </tr>
        <tr>
            <td colspan="2"><pre>
CREATE INDEX notification_index ON notification USING btree(client, notification_date);</td>
        </tr>
    </tbody>
</table>
...
<table>
	<tbody>
        ...
        <tr>
            <th> Justification </th>
            <td> Used for improving the performance of full text search, while searching for a client. We used GIN because a client's name is not updated frequently.</td>
        </tr>
        <tr>
            <td colspan="2"><pre>
CREATE INDEX search_client ON client USING GIN (search);</td>
        </tr>
    </tbody>
</table>
...
<table>
	<tbody>
        ...
        <tr>
            <th> Justification </th>
            <td> Used for improving the performance of full text search, while searching for a project. We used GIN because a project's name is not updated frequently.</td>
        </tr>
        <tr>
            <td colspan="2"><pre>
CREATE INDEX search_project ON project USING GIN (search);</td>
        </tr>
    </tbody>
</table>
...
<table>
	<tbody>
        ...
        <tr>
            <th> Justification </th>
            <td> Used for improving the performance of full text search, while searching for a task. We used GIN because a task's name and description is not updated frequently.</td>
        </tr>
        <tr>
            <td colspan="2"><pre>
CREATE INDEX search_task ON task USING GIN (search);</td>
        </tr>
    </tbody>
</table>
...
<table>
	<tbody>
        ...
        <tr>
            <th> Description </th>
            <td> When creating a client or when he updates his name, must change FTS</td>
        </tr>
        <tr>
            <td colspan="2"><pre>
...
BEGIN
    IF TG_OP = 'INSERT' THEN
        NEW.search = (SELECT setweight(to_tsvector(account.username), 'A')  || setweight(to_tsvector(account.email), 'A') || setweight(to_tsvector(coalesce(NEW.fullname, '')), 'B') || setweight(to_tsvector(coalesce(NEW.company, '')), 'C') FROM account WHERE NEW.id=account.id);
...
CREATE TRIGGER update_client_search 
    ...
    ON client
    FOR EACH ROW 
    EXECUTE PROCEDURE client_search_update();</td>
        </tr>
    </tbody>
</table>
...
<table>
	<tbody>
        ...
        <tr>
            <th> Description </th>
            <td> When creating a project or when its name is updated, must change FTS</td>
        </tr>
        <tr>
            <td colspan="2"><pre>
...
CREATE TRIGGER update_project_search 
    ...
    ON project
    FOR EACH ROW 
    EXECUTE PROCEDURE project_search_update();</td>
        </tr>
    </tbody>
</table>
...
<table>
	<tbody>
        ...
        <tr>
            <th> Description </th>
            <td> When creating a task or when its name is updated, must change FTS</td>
        </tr>
        <tr>
            <td colspan="2"><pre>
...
CREATE TRIGGER update_task_search 
    ...
    ON task
    FOR EACH ROW 
    EXECUTE PROCEDURE task_search_update();</td>
        </tr>
    </tbody>
</table>
...
<table>
	<tbody>
        ...
        <tr>
            <th> Description </th>
            <td> A tag can only be added to a task that belongs to the same project.</td>
        </tr>
        <tr>
            <td colspan="2"><pre>
...
CREATE TRIGGER assign_tag
    ...
    FOR EACH ROW 
    EXECUTE PROCEDURE assign_tag(); </td>
        </tr>
    </tbody>
</table>
...
<table>
	<tbody>
        ...
        <tr>
            <th> Description </th>
            <td> A client can only be assigned to task of a project that he is a team member.</td>
        </tr>
        <tr>
            <td colspan="2"><pre>
...
$BODY$
BEGIN
    IF NOT EXISTS (SELECT * FROM team_member, task WHERE NEW.task = task.id AND NEW.client = team_member.client_id AND team_member.project_id = task.project) THEN
...
CREATE TRIGGER assign_member
    ...
    FOR EACH ROW 
    EXECUTE PROCEDURE assign_member(); </td>
        </tr>
    </tbody>
</table>
...
<table>
	<tbody>
        ...
        <tr>
            <th> Description </th>
            <td> A project needs to have at least one Owner. </td>
        </tr>
        <tr>
            <td colspan="2"><pre>
...
$BODY$
BEGIN
    IF OLD.member_role = 'Owner' AND (SELECT count(*) FROM team_member WHERE project_id = OLD.project_id AND member_role = 'Owner') = 1 AND (SELECT COUNT(*) FROM team_member WHERE project_id = OLD.project_id) > 1
    THEN
...
CREATE TRIGGER check_project_owner
    ...
    FOR EACH ROW
EXECUTE PROCEDURE check_project_owner(); </td>
        </tr>
    </tbody>
</table>
...
<table>
	<tbody>
        ...
        <tr>
            <th> Description </th>
            <td> When a user accepts an invite, he joins the becomes a project team member. </td>
        </tr>
        <tr>
            <td colspan="2"><pre>
...
CREATE TRIGGER accept_invite
    ...
    ON invite
    FOR EACH ROW
EXECUTE PROCEDURE accept_invite(); </td>
        </tr>
    </tbody>
</table>
...
<table>
	<tbody>
        ...
        <tr>
            <th> Description </th>
            <td> A task's due date must be equal or lesser than project's due date. </td>
        </tr>
        <tr>
            <td colspan="2"><pre>
...
CREATE TRIGGER check_task_date
    ...
    FOR EACH ROW 
    EXECUTE PROCEDURE check_task_date(); </td>
        </tr>
    </tbody>
</table>
...
<table>
	<tbody>
        ...
        <tr>
            <th> Description </th>
            <td> A subtask's due date must be equal or lesser than its parent's due date. </td>
        </tr>
        <tr>
            <td colspan="2"><pre>
...
CREATE TRIGGER check_sub_date
    ...
    FOR EACH ROW 
    EXECUTE PROCEDURE check_sub_date(); </td>
        </tr>
    </tbody>
</table>
...
<table>
	<tbody>
        ...
        <tr>
            <th> Description </th>
            <td> A notification is added when a client is invited to a project. </td>
        </tr>
        <tr>
            <td colspan="2"><pre>
...
CREATE TRIGGER add_invite_notification
    ...
    ON invite
    FOR EACH ROW
EXECUTE PROCEDURE add_invite_notification(); </td>
        </tr>
    </tbody>
</table>
...
<table>
	<tbody>
        ...
        <tr>
            <th> Description </th>
            <td> A notification is added when a client enters a project. </td>
        </tr>
        <tr>
            <td colspan="2"><pre>
...
CREATE TRIGGER add_project_notification
    ...
    ON team_member
    FOR EACH ROW
EXECUTE PROCEDURE add_project_notification(); </td>
        </tr>
    </tbody>
</table>
...
<table>
	<tbody>
        ...
        <tr>
            <th> Description </th>
            <td> A notification is added when a team member is assigned to a task. </td>
        </tr>
        <tr>
            <td colspan="2"><pre>
...
CREATE TRIGGER add_assignment_notification
    ...
    ON assignment
    FOR EACH ROW
EXECUTE PROCEDURE add_assignment_notification(); </td>
        </tr>
    </tbody>
</table>
...
<table>
	<tbody>
        ...
        <tr>
            <th> Description </th>
            <td> A notification is added when a team member comments on a task. </td>
        </tr>
        <tr>
            <td colspan="2"><pre>
...
CREATE TRIGGER add_comment_notification
    ...
    ON comment
    FOR EACH ROW
EXECUTE PROCEDURE add_comment_notification(); </td>
        </tr>
    </tbody>
</table>
...
<table>
	<tbody>
        ...
        <tr>
            <th> Description </th>
            <td> A notification is added when a report is reviewed by an admin. </td>
        </tr>
        <tr>
            <td colspan="2"><pre>
...
CREATE TRIGGER add_report_notification
    ...
    ON report
    FOR EACH ROW
EXECUTE PROCEDURE add_report_notification(); </td>
        </tr>
    </tbody>
</table>
...
<table>
	<tbody>
        ...
        <tr>
            <th> Justification </th>
            <td> In order to maintain consistency, it's necessary to use a transaction to ensure that the all the code executes without errors. If an error occurs, a ROLLBACK is issued (when the insertion of a project fails, per example). The isolation level is Serializable , because we need an execution of operations in which concurrently executing transactions appears to be serially executing. </td>
        </tr>
        <tr>
...
-- Create subtask
 ...
 VALUES ($name, $description, $due_date)
 RETURNING id;
<br>
-- Insert subtask relation
 INSERT INTO subtask (id, parent) 
 VALUES (id, $parent);
<br>
COMMIT;</td>
        </tr>
...
<br>
-- Create comment
 INSERT INTO comment (task, author, comment_text) 
 VALUES ($task, $author, $comment_text)
 RETURNING id;
<br>
-- Insert comment_reply relation
 INSERT INTO comment_reply (id, parent) 
 VALUES (id, $parent);
<br>
COMMIT;</td>
        </tr>
...
[Create.sql File](https://git.fe.up.pt/lbaw/lbaw2021/lbaw2134/-/blob/master/sql/create.sql)

[Populate.sql File](https://git.fe.up.pt/lbaw/lbaw2021/lbaw2134/-/blob/master/sql/populate.sql)
...
DROP TRIGGER IF EXISTS check_sub_date ON task;
...
* Removed **admin**, **subtask** and **comment_reply** tables
* Updated Indexes, Triggers, Create and Populate

***
GROUP2134, 18/04/2021
